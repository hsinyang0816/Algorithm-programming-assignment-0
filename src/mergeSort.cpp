...
void MergeSort::merge( int left, int middle1, int middle2, int right ) 
{
  ...
  vector<int> leftvec(data.begin() + left, data.begin() + middle2);  
  vector<int> rightvec(data.begin() + middle2, data.begin() + right + 1);  
  ...
  leftvec.push_back( std::numeric_limits<int>::max());
  rightvec.push_back( std::numeric_limits<int>::max());
